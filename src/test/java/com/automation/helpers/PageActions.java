...
public class PageActions extends BasePage {
        ...
        if (ele.size() > 1) {
            ...
            while (moreDealsLeftToSwipe) {
                while (counter < maxSwipesForCard) {
                    ...
                    new SwipeActions().slideTheCardFromRightToLeft(ele.get(1));
                    Thread.sleep(500);
    ...
    public class SwipeActions {
        public void slideTheCardFromRightToLeft(MobileElement ele) {
            scroll(ele.getSize().getWidth(), ele.getCenter().getY(), ele.getSize().getWidth() * (1 / 4), ele.getCenter().getY());
        }
        ...
        public void slideElementFromLeftToRight(MobileElement ele) {
            Point pt = ele.getLocation();
            scroll(pt.getX(), pt.getY(), pt.getX() + ele.getSize().getWidth(), pt.getY());
        }
        ...
        public void slidePartiallyAcrossTheCardFromRightToLeft(MobileElement ele) {
            Point pt = ele.getLocation();
            scroll((int) (ele.getSize().getWidth() * 0.98), pt.getY() + (ele.getSize().getHeight() / 10), (int) (ele.getSize().getWidth() * 0.50), pt.getY() + (ele.getSize().getHeight() / 10));
        }
    }
}
