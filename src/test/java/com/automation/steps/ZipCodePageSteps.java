...
public class ZipCodePageSteps {
    ...
    @And("^click to update the zipcode")
    public void clickToupdatezipcode() {
        zipCodePage.clicktoUpdateZipcode();
    }
 ...
 /*   @Then("^zipcode page is displayed$")
    ...
    @When("^user enters vons \"([^\"]*)\" in store zipcode page$")
    public void user_enters_vons_instore_zipcode_page(String zipcode) throws Throwable {
        if (zipcode.equalsIgnoreCase("zipcode")) {
            ...
            zipCodePage.enterVonsStoreZipCode("zipcode");
        }
    }
    ...
    @And("^if user is on zipcode page then user enters \"([^\"]*)\"$")
    public void if_user_is_on_zipcode_page_then_user_enters_something(String zipcode) {
        if (isZipcodeScreenDisplayed()) {
            String regex = "\\d+";
            if (zipcode.equalsIgnoreCase("zipcode_diff")) {
                zipCodePage.enterZipCode(device.getConfigureData(BannerConfType.BANNER_ZIPCODE_DIFF));
    ...
    public boolean isZipcodeScreenDisplayed() {
        return zipCodePage.isZipcodePageDisplayed();
    }
